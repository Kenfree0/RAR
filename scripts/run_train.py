...
if __name__ == '__main__':
    ...
    parser.add_argument('--schedule', type=str, default='cosine', choices=['linear', 'cosine', 'sqrt', 'trunc_cos', 'trunc_lin', 'pw_lin', 'warmup-decay'])
    parser.add_argument('--steps', type=int, default=4000, help='steps')
    parser.add_argument('--sampler', type=str, default='uniform', choices=['uniform', 'lossaware', 'fixstep'])
    parser.add_argument('--learned_mean_embed', default=False, type=str2bool)
    parser.add_argument('--decode', default=False, type=str2bool)
    parser.add_argument('--reg_rate', default=0.0, type=float, )
    parser.add_argument('--decode_rate', default=0.2, type=float, help='max decode rate of [MASK]')

    parser.add_argument('--seq_len', type=int, default=128, help='max len of input sequence')
    parser.add_argument('--hiddecon_t_dim', type=int, default=128, help='hidden size of time embedding')
    parser.add_argument('--hidden_dim', type=int, default=128, help='hidden size of word embedding')
    parser.add_argument('--learning_steps', type=int, default=400, help='total steps of learning')
    ...
    COMMANDLINE = f"python train.py   " \
                  ...
                  f"--steps {args.diff_steps} " \
                  f"--schedule {args.noise_schedule} " \
                  f"--sampler {args.schedule_sampler} --resume_checkpoint {args.resume_checkpoint} " \
